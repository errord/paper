...
最小方差组合（Minimum Variance Portfolio）完全不使用期望收益估计，仅最小化组合方差（Jagannathan & Ma, 2003）：
...
**理论基础：** Jagannathan与Ma（2003）证明，当加入权重上界约束时，约束最小方差组合等价于对Ledoit-Wolf收缩估计的协方差矩阵做无约束最小方差优化——即**约束本身隐式执行了收缩**。当资产间的真实Sharpe比率差异较小时（DeMiguel et al., 2009），最小方差组合在样本外表现优于均值方差组合，因为它完全回避了μ估计误差这一最大的不确定性来源。

**与MV的对比：** MV需要同时估计μ和Σ；MinVar仅需Σ。在期望收益难以准确估计的市场中（如A股），MinVar理论上更具优势。

### 最大分散化模型（v3新增）

最大分散化组合（Maximum Diversification Portfolio, Choueifaty & Coignard, 2008）最大化分散化比率（Diversification Ratio）：

$$
\max_{\mathbf{w}} \quad DR(\mathbf{w}) = \frac{\mathbf{w}^\top \boldsymbol{\sigma}}{\sqrt{\mathbf{w}^\top \boldsymbol{\Sigma} \mathbf{w}}}
$$

其中 $\boldsymbol{\sigma} = (\sigma_1, \ldots, \sigma_n)^\top$ 为各资产标准差向量。

**等价形式：** Choueifaty, Froidure与Reynier（2013）证明，MaxDiv等价于在**相关矩阵**上做最小方差优化：

$$
\min_{\mathbf{w}} \quad \mathbf{w}^\top \mathbf{C} \mathbf{w} \quad \text{s.t.} \quad \mathbf{w}^\top \mathbf{1} = 1, \quad 0 \leq w_i \leq \beta_{max}
$$

其中 $\mathbf{C} = \text{diag}(\boldsymbol{\sigma})^{-1} \boldsymbol{\Sigma} \text{diag}(\boldsymbol{\sigma})^{-1}$ 为相关矩阵。

**理论意义：**
- DR=1表示完全不分散（单一资产），DR越大分散化越充分
- MaxDiv不需要期望收益估计，仅依赖相关矩阵
- 相比MinVar在协方差空间优化，MaxDiv在相关性空间优化，更关注**分散化结构**而非绝对波动率
- Choueifaty等（2013）在全球多市场实证中证明MaxDiv的样本外Sharpe比率优于等权和最小方差

**与MinVar的互补性：** MinVar最小化组合方差，倾向于集中配置低波动资产；MaxDiv最大化分散化比率，倾向于在低相关资产间分散。在高相关市场（如A股熊市），两者可能产生不同的权重结构。
